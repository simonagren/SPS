...
import { ActivityHandler, BotState, StatePropertyAccessor, ConversationState, UserState, CardFactory, TurnContext, Activity } from 'botbuilder';
...
export class ProvisionBot extends ActivityHandler {
    ...
    constructor(conversationState: BotState, userState: BotState, dialog: Dialog, logger: Logger, conversationReferences: any, socket: SocketIOClient.Socket) {
        ...
        if (!socket) {
            throw new Error('[socket]: Missing parameter. socket is required');
        }
        ...
        this.onMembersAdded(async (context, next) => {
            ...
            for (const member of membersAdded) {
                if (member.id !== context.activity.recipient.id) {
                    ...
                    // this._registerSocketIo(context);
                }
            }
        ...
        this.onMessage(async (context, next) => {
            ...
            if (context.activity.type === 'message') {
                ...
                if (activity.text === undefined && activity.value && activity.value.isFromAdaptiveCard && activity.value.messageText) {
