...
const activityFunction: AzureFunction = async function (context: Context, queueObj: IQueueObj): Promise<string> {
    ...
    try {
        ...
        // Emit update
        service.emitUpdate(false, "Creating Content Types", room, socket, conversationId);
        ...
        ps.addCommand(`Apply-PnPProvisioningTemplate -Path C:/Users/sagren/dev/sps/Azure/FunctionApp/${inputs.siteType}-ctypes.xml`);
        // ps.addCommand(`Apply-PnPProvisioningTemplate -Path D:/home/site/wwwroot/FunctionApp/${inputs.siteType}-ctypes.xml`);
        
        output = await ps.invoke();
        context.log(output);
        
        // Emit update
        service.emitUpdate(false, "Successfully created Content Types", room, socket, conversationId);

        return "Created Content Types";
